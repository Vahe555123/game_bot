    ...
    async def get_purchase_price_rub(
        ...
        if payment_url:
            ...
            if parsed_price is not None:
                logger.info(
                    f"Parsed Turkey purchase price from payment page: "
                    f"{purchase_info.total_value} TL -> {parsed_price} RUB"
                )
                return parsed_price
        ...
        for denomination, quantity in purchase_info.quantity_map.items():
            ...
            if unit_price_rub is None:
                logger.warning(
                    f"Unable to resolve Turkey card price for denomination "
                    f"{denomination} TL while calculating total purchase price"
                )
                return None
        ...
        if total_price_rub > 0:
            logger.info(
                f"Calculated Turkey purchase price from denominations: "
